...
def convert_to_database_date(czech_date):
    try:
        # Převod českého data na datetime objekt
        czech_date = czech_date.strip()
        date_obj = datetime.strptime(czech_date, "%d. %m. %Y")
        # Převod na databázový formát
...
database_date = convert_to_database_date(czech_date)
print(database_date)


def generate_identifiers(input_phrase, use_camel_case=True):
    # Odstranění diakritiky
    input_phrase = input_phrase.translate(str.maketrans("", "", "áčďéěíňóřšťúůýžÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ"))

    # Rozdělení na slova
    words = input_phrase.split()

    # Převedení slov na identifikátory
    if use_camel_case:
        identifiers = [word.lower() if i == 0 else word.title() for i, word in enumerate(words)]
    else:
        identifiers = ["_".join(word.lower().split()) for word in words]
...
python_identifier = generate_identifiers(phrase, use_camel_case=False)
js_camel_case = generate_identifiers(phrase, use_camel_case=True)
print("Python identifikátor:", python_identifier)
print("JS CamelCase identifikátor:", js_camel_case)


import random
import string

def generate_passwords(num_passwords, length=10):
    passwords = []
    for _ in range(num_passwords):
        upper_case = ''.join(random.choices(string.ascii_uppercase, k=3))
        lower_case = ''.join(random.choices(string.ascii_lowercase, k=3))
        special_char = random.choice("-+*")
        digits = ''.join(random.choices(string.digits, k=3))
        password = upper_case + lower_case + special_char + digits
